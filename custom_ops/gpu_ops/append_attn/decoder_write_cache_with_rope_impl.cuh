...
//   constexpr int THREADS_PER_HEAD = HeadDim / 2 / VecSize;
//   constexpr int K_VEC_SIZE = 4;
//   constexpr int HALF_K_VEC_SIZE = 2;
//   constexpr int NUM_K_VEC_PER_HEAD = THREADS_PER_HEAD / K_VEC_SIZE;
//   using LoadEmbT = AlignedVector<float, VecSize>;
...
//   const int tid_in_current_head = tid % THREADS_PER_HEAD;
//   const int rank_in_current_head = tid / THREADS_PER_HEAD;
...
//         #pragma unroll
//         for (int i = 0; i < VecSize; i++) {
//           local_max = __hmax(local_max, __habs(left_bias_vec[i]));
//           local_max = __hmax(local_max, __habs(right_bias_vec[i]));
//         }
//         #pragma unroll
//         for (int m_offset = THREADS_PER_HEAD / 2; m_offset > 0; m_offset /=
//         2){
//           local_max =
//             __hmax(local_max, __shfl_xor_sync(0xffffffff, local_max,
//             m_offset, THREADS_PER_HEAD));
//         }
//         scale = __hdiv(448, local_max);
//         int cache_offset = block_idx * kv_num_heads * block_size +
//                        kv_head_idx * block_size +
//                        block_offset;
//         T* cache_k_scale_now = cache_k_scale + cache_offset;
//         T* cache_v_scale_now = cache_v_scale + cache_offset;
//         if (tid_in_current_head == 0) {
//           if (hi < num_heads + kv_num_heads) {
...
//       // quant key and value
//       for (int i = 0; i < VecSize; i++) {
//         left_cache_vec[i] = QuantToC8<T, true, IsFP8, RoundType>(
//           scale, left_bias_vec[i], 448.0, -448.0);
//         right_cache_vec[i] = QuantToC8<T, true, IsFP8, RoundType>(
//           scale, right_bias_vec[i], 448.0, -448.0);
//       }

//       // store
//       if (hi < num_heads + kv_num_heads) {
//         // dynamic quant fp8 for every key head, every head has one scale.
//         for(int ii=0; ii < NUM_K_VEC_PER_HEAD; ii++){
//           const store_offset = (tid_in_current_head*NUM_K_VEC_PER_HEAD+ii);
//           const int start_block_16 =
//             block_offset / 16 * 16 + block_offset % 8 + store_offset % 2 * 8;
...
//             store_offset / 2 * 32 + (block_offset % 16) / 8 * 16 +
//             (tid_in_current_head%2+1) * 4;
//           Store<uint8_t, K_VEC_SIZE>(&left_cache_vec[K_VEC_SIZE*ii],
//           &key_cache[tgt_cache_idx]); Store<uint8_t,
//           K_VEC_SIZE>(&right_cache_vec[K_VEC_SIZE*ii],
//           &key_cache[tgt_idx_right]);
//         }
//       } else {
//         // dynamic quant fp8 for every value head, every head has one scale.
//         Store<uint8_t, VecSize>(left_cache_vec, &value_cache[tgt_idx_left]);
//         Store<uint8_t, VecSize>(right_cache_vec,
//         &value_cache[tgt_idx_right]);
//       }
...
template <typename T,
          ...
          bool IsFP8 = true,
          bool IsDynamic = true>
__global__ void append_decode_cache_T_fp8_neox_rope_kernel(
    const T* __restrict__ quant_qkv,    // [bsz, num_heads + 2 * kv_num_heads,
                                        // head_size]
    uint8_t* __restrict__ key_cache,    // [num_blocks, kv_num_heads,
...
#pragma unroll
      for (uint32_t head_bias = lane_id * VecSize; head_bias < HeadDim;
        ...
        // q rope
        const uint32_t emb_idx = write_seq_id * HeadDim + head_bias % head_size;
...
#pragma unroll
        for (int i = 0; i < VecSize; i++) {
          ...
          float tmp1 = input_left * cos_tmp - input_right * sin_tmp;
          float tmp2 = input_right * cos_tmp + input_left * sin_tmp;
          src_vec[i] = static_cast<T>(tmp1);
          src_vec_right[i] = static_cast<T>(tmp2);
        }
        Store<T, VecSize>(out_vec, &qkv_out_now[bias_idx]);
        Store<T, VecSize>(src_vec_right,
                          &qkv_out_now[bias_idx + half_head_size]);
      }
    }
    ...
    if (head_idx < num_heads + kv_num_heads) {
      const uint32_t emb_idx = write_seq_id * half_head_size + head_bias;
      const uint32_t new_emb_idx =
          rope_3d ? emb_idx + bid * max_seq_len * HeadDim : emb_idx;
